...
function _update(nbpath, updatedNb) {
    _get(nbpath).then(function(nb) {
        var nbAbsPath = _cache[nbpath].absPath;
        return new Promise(function(resolve, reject) {
            fs.writeFile(nbAbsPath, JSON.stringify(updatedNb), function(err) {
                if (err) {
                    reject(err);
                } else {
                    _uncache(nbpath);
