    ...
    return new Promise(function(resolve, reject) {
        fs.mkdir(path.dirname(destination), function(err) {
            if (err && err.code !== 'EEXIST') {
                ...
                // copy dashboard server-set metadata from current version
                nbmetadata.copyDashboardServerMetadata(buffer, destination)
                    .then(function success(updatedBuffer) {
                        fs.writeFile(destination, updatedBuffer, function(err) {
                            if (err) {
                                reject(err);
                            } else {
                                resolve();
...
function _writeZipFile(destination, buffer) {
    ...
    .then(function(tempobj) {
        ...
        .then(function(tmpUnzipDir) {
            return new Promise(function(resolve, reject) {
                var indexFile = path.join(tmpUnzipDir, INDEX_NB_NAME);
                fs.readFile(indexFile, ENCODING, function(err, rawData) {
                    if (err) {
                        reject(err);
                    } else {
                        nbmetadata.copyDashboardServerMetadata(rawData, destination)
                            .then(
                                function success(updatedData) {
                                    fs.writeFile(indexFile, updatedData, function(err) {
                                        if (err) {
                                            reject(err);
                                        } else {
                                            resolve(tmpUnzipDir);
